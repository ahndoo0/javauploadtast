...
public class FirstAspect {
    ...
    public void afterReturning( JoinPoint jp, ModelProduct product ) {
        ...
        logger.debug("afterReturning ---> 메서드 호출 후"+ jp.getSignature().getName());
        logger.debug("afterReturning ---> 메서드 호출 후"+ jp.getArgs().toString());
    }
    ...
    //afterThrowing : 메서드 실행시 예외가 발생 했을때
    public void afterThrowing( Throwable e ){
        logger.debug("afterThrowing ---> 메서드 실행 중 예외 발생");
        logger.debug("afterThrowing ---> exception" + e.getMessage());
        logger.debug("afterThrowing ---> 메서드 실행 중 예외 발생");
    }
}
